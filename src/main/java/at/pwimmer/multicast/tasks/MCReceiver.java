...
public class MCReceiver implements Runnable {
    ...
    public void run() {
        ...
        try(MulticastSocket socket = new MulticastSocket(port)) {
            ...
            System.err.println("Failed to receive data from ["+group+"::"+port+"]");
            shutdown();
        }
    }
    ...
    private void receiveData(MulticastSocket socket) throws IOException {
        if(!socket.isClosed()) {
            try {
                ...
                long counter = packets.incrementAndGet();

                buffer = Arrays.copyOfRange(packet.getData(), packet.getOffset(), packet.getOffset()+packet.getLength());
                outputData(buffer);
        ...
        else {              // Otherwise, the passed socket has already been closed.
            shutdown();     // If so, then also shutdown the task.
        }
    }
    ...
    private void outputData(byte[] buffer) {
        if(buffer != null && buffer.length > 0) {
            ...
            switch(output) {
                case ASCII:     data = new String(buffer, Charset.forName("UTF-8")); break;
                case BINARY:
